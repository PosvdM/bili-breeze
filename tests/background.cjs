...
(async()=>{
  ...
  const invalid=await send({type:'detect',state:state('invalid')});
  assert.equal(invalid.ok,false);assert.equal(invalid.retry,false,'malformed answer is not retried automatically');
  responseProb=.91;
  assert.equal((await send({type:'detect',state:state('after-invalid')})).ok,true,'malformed answer does not pause requests');
  status=503;
  const unavailable=await send({type:'detect',state:state('unavailable')});
  assert.equal(unavailable.ok,false);assert.equal(unavailable.retry,true,'service outage is retried later');
  const count=fetchCount;
  const paused=await send({type:'detect',state:state('cooldown')});assert.equal(fetchCount,count);
  assert.equal(paused.retry,true,'cooldown rejection is retried later');
  ...
  set({whitelist:['uid:42']});const space={id:'test',url:'https://space.bilibili.com/42/dynamic'};
  assert.equal((await send({type:'spaceList'},space)).data.whitelist,true,'legacy uid string shown as listed');
  assert.equal((await send({type:'spaceList',list:'whitelist',add:false},space)).data.whitelist,false);
  assert.deepEqual(store.whitelist,[],'legacy uid string removed');
  set({whitelist:[]});

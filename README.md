...
输出格式由扩展固定附加，不能修改；抽奖主次的判断仍使用内置规则。修改 Prompt 后，内容会重新识别，并使用你的 API 额度；恢复内置 Prompt 后沿用原有缓存。

从 1.0.3 升级时，已填写的补充规则会自动接在内置 Prompt 末尾，识别效果不变。
...
密钥仅保存在本机。识别会把文字、标题和内容链接发送给所选服务商，使用你的 API 额度。扩展不分析图片或视频。
...
扩展包生成在 `dist/`，可直接拖入扩展管理页安装。测试使用模拟 API 响应，覆盖分类规则、名单、缓存、页面折叠和弹窗设置；不代表真实模型的分类准确率。详见 [测试记录](TESTING.md)。
